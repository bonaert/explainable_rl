...
class LunarLander(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        if (self.continuous and np.abs(action[1]) > 0.5) or (not self.continuous and action in [1, 3]):
            ...
            impulse_pos = (
            self.lander.position[0] + ox - tip[0] * 17 / SCALE, self.lander.position[1] + oy + tip[1] * SIDE_ENGINE_HEIGHT / SCALE)
    ...
    def render(self, mode='human', goal=None):
        ...
        def state_to_vals(x, y, vel_x, vel_y, angle, angular_momentum, contact_left, contact_right):
            # (pos.x - VIEWPORT_W / SCALE / 2) / (VIEWPORT_W / SCALE / 2),
            # (pos.y - (self.helipad_y + LEG_DOWN / SCALE)) / (VIEWPORT_H / SCALE / 2),
            # vel.x * (VIEWPORT_W / SCALE / 2) / FPS,
            # vel.y * (VIEWPORT_H / SCALE / 2) / FPS,
            # self.lander.angle,
            # 20.0*self.lander.angularVelocity/FPS,
            new_x = x * (VIEWPORT_W / SCALE / 2) + VIEWPORT_W / SCALE / 2
            new_y = y * (VIEWPORT_H / SCALE / 2) + (self.helipad_y + LEG_DOWN / SCALE)
            new_vel_x = vel_x * FPS / (VIEWPORT_W / SCALE / 2)
            new_vel_y = vel_y * FPS / (VIEWPORT_H / SCALE / 2)
            new_angle = angle
            new_angular_momentum = angular_momentum * FPS / 20.0
