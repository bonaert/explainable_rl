...
def evaluate_hac(hac_params: HacParams, env: gym.Env, goal_state: np.ndarray,
                 num_evals: int = 5, always_render: bool = False) -> Tuple[int, float]:
    num_successes = 0
    for i in range(num_evals):
        state = env.reset()
        _, failed = run_hac(hac_params, state, goal_state, env, training=False, render=(i == 0 or always_render))
...
def train(hac_params: HacParams, env: gym.Env):
    for i in tqdm(range(hac_params.num_training_episodes)):
        ...
        if i == 0 or (i + 1) % hac_params.evaluation_frequency == 0:
            num_successes, success_rate = evaluate_hac(hac_params, env, goal_state)
    ...
    with open(f'{directory}/hac_params.json', 'r') as f:
        ...
        for key, value in hac_params_as_dict.items():
            if type(value) == list and key not in ["policies", "her_storage"]:
                hac_params_as_dict[key] = np.array(value)
...
if __name__ == '__main__':
    ...
    current_hac_params = HacParams(
        ...
        # https://github.com/nikhilbarhate99/Hierarchical-Actor-Critic-HAC-PyTorch/blob/117d4002e754a53019b5cf7f103946d382488217/utils.py#L4
        subgoal_testing_frequency=0.3,
        # https://github.com/andrew-j-levy/Hierarchical-Actor-Critc-HAC-/blob/f90f2c356ab0a95a57003c4d70a0108f09b6e6b9/design_agent_and_env.py#L125
        ...
        # https://github.com/andrew-j-levy/Hierarchical-Actor-Critc-HAC-/blob/f90f2c356ab0a95a57003c4d70a0108f09b6e6b9/agent.py#L40
