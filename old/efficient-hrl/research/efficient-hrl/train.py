...
def collect_experience(tf_env, agent, meta_agent, state_preprocess,
  ...
  """
    ...
    1) Get the current state, get an action (given that state), take a step to get the reward, discount and transition type
    2) Preprocess the state
    3) Check if the transition_type is TRANSITION or FINAL_TRANSITION.
    4) Check if the transition_type is RESTARTING
    5) Do the increment step, increment episode and increment reset op (what is that last one?)
    6) Get the next state and preprocess it
    7) Define an "next_reset_episode_cond" op that is true if the discount is 0 or the env is restarting
    8) Optionally store the context (why?)
    9) ?? Bizarre code about collecting experience and updating episode reward if the env isn't restarting ??
    10) After doing the collect experience ops:
        1) Define the variables for the states, actions, state, reward, meta action and meta context
        2) Update the action variable (with the action of the meta controller) at a specific index (which I don't get?)
        3) Define weird "actions" and "meta_reward" tensors
        4) Define the reward as 10% of the meta reward
        5) Define a big "meta_transition" with all the useful info (state, meta action, reward, states, actions)
        6) Optionally add the NOMETA context to the NOMETA transtion and the META context to the META transition
        7) Define a "meta stop cond" op that is True if there is a transition in the env and either (it's the
           end of the meta transition) OR (the env is restarting and the discount is 0) - This is very weird, why?
        8) Add the transition to the NOMETA replay buffer (if there's a transition and it's not an env reset)
        9) Add the meta-transition to the META replay buffer (if there's a transition and the meta-transition ended?)
    11) Reset the environment if needed; and then get the time step
    12) Update the states variable to include the latest state (it's a circular buffer)
    13) Update the state variable to be equal to the next state it's time to do a meta action, else don't change it
    14) Update the reward variable: to 0 if it's a meta action, otherwise add the new reward
    15) If it's time to do a meta action, set the meta_action_var to be equal to the context vars (why?)
    16) If it's time to do a meta action, update the meta context vars to be equal to the meta agents context vars (why?)
    """
...
def sample_best_meta_actions(state_reprs, next_state_reprs, prev_meta_actions,
    ...
    # Get the best actions indexes
    best_actions = tf.argmax(fitness, 0)
    # Get the best actions (given the indexes) and the previous meta actions
